...
pub fn system_routes(state: SystemRouterState) -> Router {
    Router::new()
        ...
        .route("/api/model-bench/models", get(benchmark_models))
        .route("/api/providers/{id}/model-bench", post(benchmark_model))
...
async fn benchmark_models(
    State(state): State<SystemRouterState>,
    Extension(user): Extension<CurrentUser>,
) -> Result<Json<ApiResponse<Vec<aionui_api_types::ModelBenchModel>>>, ApiError> {
    Ok(Json(ApiResponse::ok(
        state
            .provider_service
            .benchmark_models(&user.id)
            .await
            .map_err(ApiError::from)?,
    )))
}
...
async fn benchmark_model(
    ...
    Extension(user): Extension<CurrentUser>,
    Path(id): Path<String>,
    body: Result<Json<aionui_api_types::ModelBenchRequest>, JsonRejection>,
) -> Result<axum::response::Response, ApiError> {
    let Json(input) = body.map_err(ApiError::from)?;
    let upstream = state
        .provider_service
        .benchmark(&user.id, &id, &input)
        .await
        .map_err(ApiError::from)?;
    // Dropping the response body cancels the upstream and releases the concurrency slot.
    let stream = futures_util::stream::unfold(Some(upstream), |state| async move {
        let mut upstream = state?;
        match upstream.response.chunk().await {
            Ok(Some(bytes)) => Some((Ok::<_, std::io::Error>(bytes), Some(upstream))),
            Ok(None) => None,
            Err(_) => Some((Err(std::io::Error::other("BENCH_STREAM_INTERRUPTED")), None)),
        }
    });
