...
vi.mock('@/common', () => ({
  ipcBridge: {
    dialog: {
      showOpen: { invoke: filePickerMocks.showOpen },
    },
    project: {
      resolveRef: { invoke: filePickerMocks.resolveRef },
      attachFolder: { invoke: filePickerMocks.attachFolder },
    },
    fs: {
...
import { useAttachProjectFolders } from '@/renderer/hooks/file/useAttachProjectFolders';
...
describe('send-box folder picker entry', () => {
  it('opens a dedicated directory dialog and forwards the selected folder', async () => {
    filePickerMocks.showOpen.mockReset().mockResolvedValue(['/Users/me/Specs']);
    const onFoldersSelected = vi.fn();
    ...
    act(() => result.current.openFolderSelector());

    await waitFor(() => expect(onFoldersSelected).toHaveBeenCalledWith(['/Users/me/Specs']));
    expect(filePickerMocks.showOpen).toHaveBeenCalledWith({
      properties: ['openDirectory', 'createDirectory', 'multiSelections'],
    });
  });

  it('does not add an attachment when the directory dialog is cancelled', async () => {
    filePickerMocks.showOpen.mockReset().mockResolvedValue([]);
    const onFoldersSelected = vi.fn();
    const { result } = renderHook(() =>
      useOpenFileSelector({
        onFilesSelected: vi.fn(),
    ...
    expect(onFoldersSelected).not.toHaveBeenCalled();
  });

  it('shows separate file and folder actions without changing the plus button', async () => {
    const openFileSelector = vi.fn();
    const openFolderSelector = vi.fn();
    render(
      <MemoryRouter>
        <FileAttachButton openFileSelector={openFileSelector} openFolderSelector={openFolderSelector} />
      </MemoryRouter>
    );

    fireEvent.click(screen.getByTestId('aionrs-attach-folder-btn'));
    const folderAction = await screen.findByText('Add folder to project');
    expect(screen.getByText('Add files')).toBeInTheDocument();

    fireEvent.click(folderAction);
    expect(openFolderSelector).toHaveBeenCalledOnce();
    expect(openFileSelector).not.toHaveBeenCalled();
  });
});

describe('attach selected folders to the active project', () => {
  it('stops safely when the current conversation has no project', async () => {
    resetCurrentProjectForTest();
    filePickerMocks.resolveRef.mockReset();
    filePickerMocks.attachFolder.mockReset();
    const onFoldersAdded = vi.fn();
    const errorSpy = vi.spyOn(Message, 'error').mockImplementation(() => undefined as never);
    const { result } = renderHook(() => useAttachProjectFolders({ onFoldersAdded }));

    await act(async () => result.current(['/Users/me/Specs']));

    expect(errorSpy).toHaveBeenCalledWith('conversation.explorer.attachFailed');
    expect(filePickerMocks.resolveRef).not.toHaveBeenCalled();
    expect(onFoldersAdded).not.toHaveBeenCalled();
    errorSpy.mockRestore();
  });

  it('reports a generic project binding failure without adding a broken directory ref', async () => {
    resetCurrentProjectForTest();
    setCurrentProject('project-1');
    filePickerMocks.resolveRef.mockReset().mockResolvedValue({
      file: { kind: 'local', path: '/Users/me/Specs' },
      upgraded: false,
    });
    filePickerMocks.attachFolder.mockReset().mockRejectedValue(new Error('attach failed'));
    const onFoldersAdded = vi.fn();
    const errorSpy = vi.spyOn(Message, 'error').mockImplementation(() => undefined as never);
    const { result } = renderHook(() => useAttachProjectFolders({ onFoldersAdded }));
  ...
  it('uses the overlap warning and leaves the attachment list unchanged', async () => {
    resetCurrentProjectForTest();
    setCurrentProject('project-1');
    filePickerMocks.resolveRef.mockReset().mockResolvedValue({
      file: { kind: 'local', path: '/Users/me' },
      upgraded: false,
    });
    filePickerMocks.attachFolder.mockReset().mockRejectedValue({
      name: 'BackendHttpError',
      status: 409,
      code: 'project_explorer_overlap',
    });
    const onFoldersAdded = vi.fn();
    const warningSpy = vi.spyOn(Message, 'warning').mockImplementation(() => undefined as never);
    const { result } = renderHook(() => useAttachProjectFolders({ onFoldersAdded }));

    await act(async () => result.current(['/Users/me']));

    expect(warningSpy).toHaveBeenCalledWith('conversation.explorer.attachOverlap');
    expect(onFoldersAdded).not.toHaveBeenCalled();
