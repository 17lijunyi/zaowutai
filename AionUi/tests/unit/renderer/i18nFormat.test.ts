...
describe('resolveFormatLocale', () => {
  ...
  it('falls back to the default language for unknown or empty input', () => {
    expect(resolveFormatLocale('kl-GL')).toBe('zh-CN');
    expect(resolveFormatLocale('')).toBe('zh-CN');
    expect(resolveFormatLocale(undefined)).toBe('zh-CN');
    expect(resolveFormatLocale(null)).toBe('zh-CN');
  });
});
