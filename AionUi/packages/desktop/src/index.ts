...
import { UPSTREAM_UPDATE_ENABLED } from '@/common/branding';
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  ...
  // Initialize the upstream auto-updater only for builds that explicitly opt in.
  // 定制版默认不连接上游更新源，避免覆盖本地品牌改动。
  ...
  if (!disableAutoUpdater) {
    ...
    console.log('[AionUi] Auto-updater disabled by custom-build/env/CI guard');
  }
