...
Please check your local CLI tool authentication status`,
  ...
  const appendSelectedFolderItems = useCallback(
    (additions: FileOrFolderItem[]) => {
      const merged = mergeFileSelectionItems(atPathRef.current, additions);
      if (merged !== atPathRef.current) setAtPath(merged as Array<string | FileOrFolderItem>);
    },
    [setAtPath]
  );
  const appendSelectedFolders = useAttachProjectFolders({ onFoldersAdded: appendSelectedFolderItems });
  const { openFileSelector, openFolderSelector, onSlashBuiltinCommand } = useOpenFileSelector({
    onFilesSelected: appendSelectedFiles,
    onFoldersSelected: appendSelectedFolders,
  });
