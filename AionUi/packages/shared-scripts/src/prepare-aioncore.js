...
function copyDirectorySafe(sourcePath, targetPath) {
  ensureDirectory(path.dirname(targetPath));
  // Keep relative links inside managed runtimes relative. Node otherwise
  // resolves them against the source directory, producing absolute links that
  // invalidate a macOS app bundle signature after packaging.
